...
function rts_gplus_notice() {
	...
	if ( ! empty( $_GET['rtnonce'] )
	&& ! wp_verify_nonce( sanitize_key( $_GET['rtnonce'] ), 'rtnonce' ) ) {
		return;
	}

	if ( ! current_user_can( 'manage_options' )
	|| ( ! empty( $_GET['page'] )
	&& 'rtsocial-options' === $_GET['page'] ) ) {
		return;
	}
	...
	|| 'hide' !== $site_option ) ) {
		...
		<div class="error rts_g_plus_notice">
			<p>
				<b><?php esc_html_e( 'rtSocial', 'rtSocial' ); ?>:</b> <?php esc_html_e( 'You need to add Google API key under', 'rtSocial' ); ?> <a href="<?php echo esc_url( admin_url( 'options-general.php?page=rtsocial-options&rtnonce=' . wp_create_nonce( 'rtnonce' ) ) ); ?>"><?php esc_html_e( 'rtSocial admin settings', 'rtSocial' ); ?></a> <?php esc_html_e( 'to display google+ count.', 'rtSocial' ); ?>
				<a href="#" onclick="rts_g_plus_override_notice();" style="float:right"><?php esc_html_e( 'Dismiss', 'rtSocial' ); ?></a>
			</p>
		</div>
...
function rtsocial_check( $args ) {
	...
	if ( ! empty( $args['active'] )
	&& in_array( 'gplus', $args['active'], true )
	&& empty( $args['google_api_key'] ) ) {
		if ( is_multisite() ) {
			update_option( 'rts_g_plus_notice', 'show' );
		} else {
			update_site_option( 'rts_g_plus_notice', 'show' );
...
function rtsocial_counter( $content = '' ) {
	...
	&& ! empty( $options['google_api_key'] ) ) {
		...
		$gplus_count = ( empty( $options['hide_count'] ) || 1 !== (int) $options['display_options_set'] ) ? '<div class="rtsocial-' . $options['display_options_set'] . '-count"><div class="rtsocial-' . $options['display_options_set'] . '-notch"></div><span class="rtsocial-gplus-count"></span></div>' : '';

		$gplus_layout = '<div class="rtsocial-gplus-' . $options['display_options_set'] . '">';

		if ( 'horizontal' === $options['display_options_set'] ) {
			$gplus_layout .= '<div class="rtsocial-gplus-' . $options['display_options_set'] . '-button"><a class="rtsocial-gplus-button" href= "https://plus.google.com/share?url=' . rawurlencode( get_permalink( $post->ID ) ) . '" rel="nofollow" target="_blank" title="+1: ' . $rtatitle . '"></a></div>' . $gplus_count;
		} else {

			if ( 'vertical' === $options['display_options_set'] ) {
				$gplus_layout .= $gplus_count . '<div class="rtsocial-gplus-' . $options['display_options_set'] . '-button"><a class="rtsocial-gplus-button" href= "https://plus.google.com/share?url=' . rawurlencode( get_permalink( $post->ID ) ) . '" rel="nofollow" target="_blank" title="+1: ' . $rtatitle . '"></a></div>';
			} else {
				...
				if ( 'icon' === $options['display_options_set'] ) {
					...
					if ( 'icon-count' === $options['display_options_set'] ) {
						...
						$gplus_count   = ( empty( $options['hide_count'] ) || 1 !== $options['hide_count'] ) ? '<div class="rtsocial-horizontal-count"><div class="rtsocial-horizontal-notch"></div><span class="rtsocial-gplus-count"></span></div>' : '';
						$gplus_layout .= ' <div class="rtsocial-gplus-icon-button"><a class="rtsocial-gplus-icon-link" href= "https://plus.google.com/share?url=' . rawurlencode( get_permalink( $post->ID ) ) . '" target= "_blank" title="+1: ' . $rtatitle . '"></a></div>' . $gplus_count;
					}
				}
			}
		}
...
function rtsocial( $args = array() ) {
	...
	&& ! empty( $options['google_api_key'] ) ) {
		...
		$gplus_count = ( empty( $options['hide_count'] ) || 1 !== (int) $options['hide_count'] ) ? '<div class="rtsocial-' . $options['display_options_set'] . '-count"><div class="rtsocial-' . $options['display_options_set'] . '-notch"></div><span class="rtsocial-gplus-count"></span></div>' : '';
		...
		if ( 'horizontal' === $options['display_options_set'] ) {
			...
			if ( 'vertical' === $options['display_options_set'] ) {
				...
				if ( 'icon' === $options['display_options_set'] ) {
					...
					if ( 'icon-count' === $options['display_options_set'] ) {
						$gplus_layout  = '<div class="rtsocial-gplus-icon">';
						$gplus_count   = ( empty( $options['hide_count'] ) || 1 !== (int) $options['hide_count'] ) ? '<div class="rtsocial-horizontal-count"><div class="rtsocial-horizontal-notch"></div><span class="rtsocial-gplus-count"></span></div>' : '';
...
function rtsocial_gplus_handler() {
	...
	&& 'rtsocial_gplus' === $_POST['action'] ) {
		...
		if ( ! empty( $options['google_api_key'] ) ) {
			...
			// Check for transient.
			$count = get_transient( 'g_plus_count_' . md5( $url ) );
			if ( false === ( $count ) ) {
				...
				if ( ! empty( $response ) && 200 === (int) wp_remote_retrieve_response_code( $response ) ) {
